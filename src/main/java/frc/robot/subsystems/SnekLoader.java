...
public class SnekLoader extends SubsystemBase {
  ...
  static final double MOTOR_IN_SPEED0 = 0.7;
  ...
  public void periodic() {
    ...
    switch (state) {
      ...
      case kShootBall2:
        speeds = new double[] { 0.0, 0.0, .85, .9, 1.0 };
        enableOneLimit(-1);
        break;
  
      case kShootBall1:
        speeds = new double[] { 0.0, .65, .85, .9, 1.0 };
        enableOneLimit(-1);
        break;
      case kShootBall0:
        speeds = new double[] { .3, .55, .85, .9, 1.0 }; //.3,.5,.75,1.0,1.0
        enableOneLimit(-1);
        break;
      default:
          speeds = ALL_STOP_SPEEDS;
