...
public class AutoBouncePath extends SequentialCommandGroup {
  ...
  public AutoBouncePath() {
    ...
    addCommands(
      ...
      new DriveArc(1.9, Math.toRadians(0), 0.762*2, Math.toRadians(45)),
      // new DriveStraightTrapProfile(Math.toRadians(-60), 3, 1.9, 1.9), //distance needs to be corrected
      new DriveArc(1.9, Math.toRadians(-150), 0.6*2, Math.toRadians(-45)),
      new DriveArc(1.9, Math.toRadians(0), 0.6, Math.toRadians(180)),
      new DriveStraightTrapProfile(Math.toRadians(90), 2.286*.25, 0,0)//,
